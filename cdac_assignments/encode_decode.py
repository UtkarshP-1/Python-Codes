...
another_message = "What? I don't have Caesar salad!"


def encode(message: str) -> str:
    dm = []
    for word in another_message:
        dm.extend(k for k,v in key.items() if v == word and word not in ('?',' ','!'))
        if word in ('?',' ','!'):
            dm.append(word)
    return ''.join(dm)
